...
<?php
    ...
    if(isset($authHeader)) {
      ...
      try{
        ...
        if($mode == 0 && isset($plantName)){    // type 0: 요일 day
          $sql = "INSERT INTO PLANT(UserID, GroupID, PlantName, DateCreated, LastWatered, Growth, RedTint, BlueTint, Life)
                        VALUES(?, ?, ?, CURDATE(), CURDATE(), 0, FLOOR(RAND() * 255), FLOOR(RAND() * 255), 2);";
          $stmt = $conn->prepare($sql);
          $stmt->bind_param("sis",$userID,$groupID,$plantName);
          $result = $stmt->execute();
        }  
        else if($mode == 1 && isset($plantID)){   // type 1 : 날짜 date
          $sql = "UPDATE PLANT SET GroupID = ? WHERE PlantID = ?;";
          $stmt = $conn->prepare($sql);
          $stmt->bind_param("ii",$groupID,$plantID);
          $result = $stmt->execute();
        }
        ...
        if($result === true){
          ...
          $sql = "SELECT LAST_INSERT_ID() AS plantID;";
          $result2 = mysqli_query($conn, $sql);
          $row = mysqli_fetch_assoc($result2);

          $outputFrame -> data = $row['plantID'];
        }
        else {
          //실패시 동작
