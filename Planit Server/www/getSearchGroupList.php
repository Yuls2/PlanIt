    ...
    if(isset($authHeader)) {
      ...
      try{
        ...
        $sql = "SELECT *,
                    ...
                    EXISTS(SELECT * FROM USER_GROUP AS m WHERE m.UserID = ? AND g.GroupID = m.GroupID) AS IsJoined
                FROM GROUP_ AS g WHERE g.GroupName LIKE ? OR g.GroupComment LIKE ?
        ...
        if($maxnum != -1){
            $stmt->bind_param("sssi",$userID,$strSearch,$strSearch,$maxnum);
        }
        else {
            $stmt->bind_param("sss",$userID,$strSearch,$strSearch);
        }
        $stmt->execute();
            ...
            while($row = mysqli_fetch_assoc($result)) {
                ...
                array_push($output,
                    array(
                        ...
                        'isJoined' => $row['IsJoined']==1?true:false,
                        'topMemberPlant' => $outputTopPlant
                    )
                );
            }
        } else{
