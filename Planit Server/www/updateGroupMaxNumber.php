      ...
      try{
        ...
        if(mysqli_num_rows($result) > 0){ // 쿼리 결과로 1행 이상 존재한다면
            ...
            if ($isManager) {
                ...
                if($maxNumber >= $row['MemberNumber']) {
                    $sql = "UPDATE GROUP_ SET MaxNumber = ? WHERE GroupID = ?;";
                    $stmt = $conn->prepare($sql);
                    $stmt->bind_param("ii",$maxNumber,$groupID);
                    $result = $stmt->execute();
            
                    if($result === true){
                        //성공 시 동작
                        $outputFrame -> data = 1; // 변경 완료
                    }
                    else {
                        //실패시 동작
                    ...
                    $outputFrame -> message = "현재 멤버 수보다 적게 설정할 수 없습니다.";
                }
            }
            else {
