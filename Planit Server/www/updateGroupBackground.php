    ...
    if(isset($authHeader)) {
      ...
      try{
        ...
        if(mysqli_num_rows($result) > 0){ // 쿼리 결과로 1행 이상 존재한다면
            ...
            if ($isManager) {
                $sql = "SELECT Score FROM GROUP_ WHERE GroupID = ?;";   // 그룹 인원수 가져옴
                $stmt = $conn->prepare($sql);
                ...
                $resJsonString = file_get_contents('json/backgroundInfo.json');
                $datas = json_decode($resJsonString, true);
                $price = $datas[(int)$background]["price"];    

                if($row['Score'] >= $price) {
                    $sql = "UPDATE GROUP_ SET Background = ? WHERE GroupID = ?;";
                    $stmt = $conn->prepare($sql);
                    $stmt->bind_param("ii",$background,$groupID);
                    $result = $stmt->execute();
            
                    if($result === true){
                        //성공 시 동작
                        $outputFrame -> data = 1; // 배경 변경 완료
                    }
                    else {
                        //실패시 동작
                    ...
                    $outputFrame -> data =  $price - $row['Score'];
                    $outputFrame -> result = 10;
                    $outputFrame -> message = "플래닛 포인트가 부족합니다.";
                }
            }
            else {
